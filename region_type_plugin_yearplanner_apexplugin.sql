...
'        sys.htp.p(''<tr class="''||V_CLASS_MONTH_NUM||'' ''||V_CLASS_COLUMN||''">'');',
...
'',
'                begin',
'                    execute immediate ''select ORIG_DAY_TYPE_CODE, ORIG_WEEK_DAY_CODE, REAL_DAY_TYPE_CODE, REAL_WEEK_DAY_CODE from CA_CALENDAR_ORIG_AND_REAL_VW where DATA_DAY = :1'' ',
'                      using V_DATE return into V_DAY_O_TYPE, V_DAY_O_CODE, V_DAY_N_TYPE, V_DAY_N_CODE;',
'                exception when others then',
'                    V_DAY_N_CODE :=  trim( to_char( V_DATE , ''DY'', ''NLS_DATE_LANGUAGE=AMERICAN'' ) );',
